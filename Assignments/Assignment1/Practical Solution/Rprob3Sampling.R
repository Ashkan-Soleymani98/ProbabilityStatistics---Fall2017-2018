...
for(i in 1 : 10){
  ...
  c <- 0
  m <- 0
  numOfEval <- 10 #---num of times wanting to get sample and make the average more accurate
  dividetime <- floor(log2(numOfGraphs)) #---suggesting the length of sample
  for(q in 1 : numOfEval){
    m <- 0
    a <- list()
    b <- 0
    while(b < ceiling((numOfGraphs[i] / dividetime))){
      #print(b)
      flag <- 0
